...
RiskPilot helps Sui DeFi users understand portfolio risk, generate protective strategies, execute bounded mainnet actions through DeepBook / DeepBook Predict, and store every agent decision as a verifiable audit trail on Walrus mainnet.
...
RiskPilot acts as an AI-native risk layer for this environment. It connects to a user's Sui mainnet wallet, reads portfolio and position data, calculates risk signals, generates a human-readable risk report, proposes a bounded hedge or protection strategy, and lets the user approve the action. The strategy can use DeepBook mainnet spot orders, margin-style risk controls, or DeepBook Predict-style downside protection. After execution or mainnet transaction preparation, RiskPilot stores the full decision package on Walrus mainnet so that the user, judges, or future integrations can verify what the agent saw, recommended, and executed.
...
RiskPilot can prepare or execute strategies using DeepBook mainnet primitives:

- DeepBook spot order for rebalancing
- Limit-order-based defensive execution
- DeepBook Predict-style binary protection
- Mainnet prepare-only mode for safe demos
- Live mainnet execution only after explicit wallet approval and policy checks
...
- Mainnet transaction digest, prepared transaction summary, or clearly labeled local simulation result
...
7. The user approves mainnet transaction preparation or live execution.
8. RiskPilot prepares or submits the mainnet transaction.
9. RiskPilot uploads the audit package to Walrus mainnet.
10. The user sees the transaction digest or prepared transaction summary, Walrus proof, and updated risk dashboard.
...
- DeepBook mainnet transaction preparation, with optional live execution after wallet approval
- Walrus mainnet audit upload
- Final result page with transaction digest / prepared transaction summary and Walrus blob ID
...
RiskPilot 帮助 Sui DeFi 用户理解资产组合风险，生成保护型策略，通过 DeepBook / DeepBook Predict 执行有边界的主网金融操作，并把每一次 agent 决策记录存入 Walrus 主网，形成可验证的审计轨迹。
...
RiskPilot 是一个 AI-native 的风险管理层。用户连接 Sui 主网钱包后，RiskPilot 会读取资产和仓位数据，计算风险信号，生成可读的风险报告，然后基于用户目标推荐保护策略。策略可以使用 DeepBook 主网现货订单、限价单、防御性再平衡，或者 DeepBook Predict 风格的下跌保护。执行或准备主网交易后，RiskPilot 会把完整决策包上传到 Walrus 主网，让用户可以验证 agent 当时看到了什么、推荐了什么、用户授权了什么、最终准备或执行了什么。
...
RiskPilot 可以通过 DeepBook 主网 primitive 准备或执行策略：

- DeepBook 现货订单用于再平衡
- 限价单用于防御性执行
- DeepBook Predict 风格的二元保护
- 主网 prepare-only 模式用于安全 demo
- live 主网执行必须经过明确的钱包签名和策略权限检查
...
- 策略模板和参数
- 用户授权约束
- 主网交易 digest、prepared transaction 摘要，或清晰标注的本地模拟结果
- 执行后风险评分
- 时间戳
...
7. 用户确认主网交易准备或 live 执行。
8. RiskPilot 准备或提交主网交易。
9. RiskPilot 上传审计包到 Walrus 主网。
10. 用户看到交易 digest 或 prepared transaction 摘要、Walrus proof 和更新后的风险仪表盘。
...
- 策略推荐
- 权限确认页面
- DeepBook 主网交易准备，用户明确签名后可选择 live 执行
- Walrus 主网审计包上传
- 结果页展示 transaction digest / prepared transaction 摘要和 Walrus blob ID
