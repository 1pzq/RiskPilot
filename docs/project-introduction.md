...
- Walrus 通过连接钱包签名/支付的方式保存可验证 audit package。
- StrategyReceipt 提供可选的链上收据，并在用户点击 mint 时由连接钱包签名和支付。
...
- 连接钱包为所有付费链上动作签名/付款：Walrus archive、符合条件的 live DeepBook Spot，以及可选 StrategyReceipt mint。
- 没有任何浏览器外的钱包作为默认付款方。
...
- Walrus preserves the verifiable audit package through connected-wallet signed and paid storage.
- StrategyReceipt provides optional on-chain receipt proof and is signed/paid by the connected wallet only when the user clicks mint.
...
- The connected wallet signs/pays every paid chain action: Walrus archive, eligible live DeepBook Spot, and optional StrategyReceipt mint transactions.
- No off-browser wallet is a default payer.
