...
function scoreConcentration(portfolio: PortfolioSnapshot, topAsset: { symbol: string; usdValue: number }): RiskSignal | null {
  ...
  return {
    ...
    summary: `${topAsset.symbol} is ${round(pct * 100)}% of tracked portfolio value.`,
    evidence: [
      `Largest asset: ${topAsset.symbol}`,
...
function scoreSuiDownside(portfolio: PortfolioSnapshot): RiskSignal | null {
  ...
  return {
    ...
    level: pct >= 0.65 ? 'critical' : 'high',
    category: 'price',
    summary: `SUI-related exposure is ${round(pct * 100)}% of the portfolio, so a drawdown would have visible impact.`,
    evidence: [
      `Direct SUI value: $${round(directSui)}`,
      `LP-related SUI exposure: $${round(exposure - directSui)}`,
...
function scoreStablecoinConcentration(portfolio: PortfolioSnapshot): RiskSignal | null {
  ...
  return {
    ...
    level: 'medium',
    category: 'stablecoin',
    summary: `${stable.symbol} is ${round(share * 100)}% of stablecoin positions.`,
    evidence: [
      `Stablecoin position value: $${round(stable.total)}`,
...
function scoreLendingRisk(portfolio: PortfolioSnapshot): RiskSignal | null {
  ...
  return {
    ...
    summary: 'This lending position is near the liquidation zone, so even a moderate drawdown matters.',
    evidence: [
...
function scoreLiquidityRisk(portfolio: PortfolioSnapshot): RiskSignal | null {
  ...
  return {
    ...
    level,
    category: 'lp',
    summary: `${highest.pair} position size is meaningful enough that impermanent loss can affect the ledger.`,
    evidence: [
      `LP value: $${round(highest.usdValue)}`,
      `Portfolio share: ${round(share * 100)}%`,
      `Risk label: ${highest.estimatedImpermanentLossRisk}`,
    ],
